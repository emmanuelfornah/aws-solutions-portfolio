...
| [healthlab-portal](https://github.com/emmanuelfornah/healthlab-portal) | The differentiator here is domain knowledge, not just infrastructure: a [roadmapped rules engine](https://github.com/emmanuelfornah/healthlab-portal#roadmap) for detecting duplicate/redundant lab orders (overlapping analyte panels, clinically-meaningless reorder intervals like re-running an HbA1c inside its 90-day window) — built on real clinical lab experience, not a generic CRUD idea. Backed by CI/CD (GitHub Actions) and HIPAA-aligned security patterns: Cognito auth, Step Functions orchestration, WAF/GuardDuty/Security Hub, X-Ray tracing — with an explicit scope note on what "HIPAA-aligned" does and doesn't mean here. | [healthlabportal.com](https://healthlabportal.com) |
